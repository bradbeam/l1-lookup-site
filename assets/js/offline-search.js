...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    //
    //
    // Throttling might be more appropriate, but we'll start here
    ...
    $searchInput.on('input', debounce((event) => {
      ...
      // This messes with the fuzzy search
    ...
    const render = ($targetSearchInput) => {
      ...
      const results = idx
        .query((q) => {
          ...
          // Try to match the full search term
          q.term(lunr.tokenizer(fullQuery), { boost: 100, fields: ['title'], usePipeline: true })
          q.term(lunr.tokenizer(fullQuery), { boost: 90, fields: ['title'], usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING })

          const tokens = lunr.tokenizer(fullQuery);
          tokens.forEach((token) => {
            const queryString = token.toString();
            // Try to match against each of the tokens
            q.term(lunr.tokenizer(queryString), { boost: 50, fields: ['title'], usePipeline: true })
            q.term(lunr.tokenizer(queryString), { boost: 20, fields: ['title'], usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING })
            q.term(lunr.tokenizer(queryString), { boost: 10, fields: ['title'], usePipeline: false, wildcard: lunr.Query.wildcard.LEADING | lunr.Query.wildcard.TRAILING })
            q.term(lunr.tokenizer(queryString), { boost: 1, fields: ['title'], editDistance: 2 })
          });
        })
